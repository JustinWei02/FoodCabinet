...
//Checklist!!!
//remove stuff with JPEG 
//check through code to figure out how the program listens to host app (mode 3)
//down sample the image to fit into 32kb 
//display RGB values using the high and low byte for each pixel into serial monitor
...
void setup() {
    ...
    //Write 1 to LoRa Module CS IIRC 
    pinMode(8, OUTPUT);
    digitalWrite(8, HIGH);
    ...
    while(1){
      ...
      //Serial.print("Line ");Serial.println(__LINE__); while(1);
      //Serial.println(temp);
      if (temp != 0x55){
...
else if (mode == 3)   //figure out what host app did to get here (do this without listening to host app)
{
  if (start_capture == 3)
  {
    //Flush the FIFO
  ...
  if (myCAM.get_bit(ARDUCHIP_TRIG, CAP_DONE_MASK))
  {
    ...
    char VH, VL;
    int i = 0, j = 0;
    //to sample smaller, check every other pixel? if j/2 or if i/2 should change resolution by factor of 4 (learn streaming algo) also change resolution settings 
    for (i = 0; i < 240; i++) 
    {
      for (j = 0; j < 320; j++)
      {
